...
const createCard = (book) => {
  ...
  let overview = book.overview.length < 100 ? book.overview : `${book.overview.substring(0, 100)} ...`;
...
const displayCart = () => {
  ...
  if (!cart.length) {
    cartContainer.innerHTML = '<p style="color:red"> No cart data Found</p>'
  }
  cart.forEach((book) => {
    const div = createCard(book);
...
const displayWishlist = () => {
  ...
  if (!wishlist.length) {
    wishlistContainer.innerHTML = '<p style="color:red"> No wishlist data Found</p>'
  }
